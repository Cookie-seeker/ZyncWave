...
object EqualizerManager {
    ...
    fun setEnabled(enabled: Boolean, context: Context) {
        ...
        context.getSharedPreferences("equalizer_prefs", Context.MODE_PRIVATE)
            .edit().putBoolean("eq_enabled", enabled).apply()
    }

    fun loadEnabledState(context: Context): Boolean {
        return context.getSharedPreferences("equalizer_prefs", Context.MODE_PRIVATE)
            .getBoolean("eq_enabled", true) // por defecto activado
    }
...
object EqualizerPresets {
    ...
    val presets: LinkedHashMap<String, List<Int>> = linkedMapOf(
        ...
        "Clásico" to listOf(4, 2, -2, 2, 3),
        "Rock" to listOf(5, 3, -3, 3, 4),
        "Pop" to listOf(-2, 2, 4, 2, -1),
        "Jazz" to listOf(3, 1, -1, 1, 3),
        "Dance" to listOf(6, 4, 0, 1, 3),
        "Hip-Hop" to listOf(5, 4, 1, 2, 1),
        "Vocal" to listOf(-3, -1, 3, 4, 1),
        "Sala" to listOf(-3, -2, -1, 1, 2)
    )


    fun levelsFor(genre: String, numBands: Int, minLevel: Short, maxLevel: Short): List<Short> {
        val base = presets[genre] ?: return List(numBands) { 0 }
        if (numBands <= 0) return emptyList()


        val maxRange = maxOf(maxLevel.toInt(), -minLevel.toInt(), 1200)

        return List(numBands) { index ->
            val position = if (numBands == 1) 0f
            else index * (base.size - 1) / (numBands - 1).toFloat()
            val lowIndex = position.toInt().coerceIn(0, base.size - 1)
            val highIndex = (lowIndex + 1).coerceAtMost(base.size - 1)
            val fraction = position - lowIndex
            val dbValue = base[lowIndex] + (base[highIndex] - base[lowIndex]) * fraction

            val millibels = (dbValue / 12f) * maxRange
            millibels.toInt().coerceIn(minLevel.toInt(), maxLevel.toInt()).toShort()
        }
    }
}